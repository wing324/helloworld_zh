...
- 内部类  
  ...
  >
  >- 局部内部类  
  >  在类的局部位置(如方法体)进行定义的类叫做局部内部类。  
  >  在局部内部类里面可以访问外部类的所有成员变量。  
  >  局部内部类访问方法中的局部变量的时候，该局部变量必须声明为final类型。(从生命周期上来理解，但这只是很简单的一个方面。)
  >
  >- 静态内部类  
  >  在内部类前添加static修饰符。  
  >  静态内部类可以声明static成员变量，非静态内部类中不可以；  
  ...
  >
  >  // 创建静态内部类的形式
  >  Outer.Innner i=new Outer.Inner();
  >  ```
...
- 异常  
  ...
  ```java
  try: 可能发生异常的代码包起来，当发生异常时，将异常抛出
  catch：捕获异常并处理
  finnal：不管是否发生异常，都会执行
  throw： 手动引发一个异常
  throws： 定义任何被调用方法异常
  ```

- 异常的分类  
  Error: Java运行时的内部错误以及资源耗尽错误，很难恢复，不期望用户处理。  
  Exception:  分为RuntimeError、非RuntimeError

- 获取异常信息  
  程序发生异常的时候，程序直接从异常处跳转到catch，try块里面异常代码之后的代码将不会再执行。

- 异常声明  
  指一个方法不处理它所产生的异常，而是调用层次向上传递，谁调用这个方法，谁来处理这个异常。

- 手动抛出异常  
  throw exception;(参数exception表示要抛出的异常对象，该对象是throwable类的子类，而且只能够是一个。

- 异常链  
  两个或者多个不同的异常出现在同一个程序中，并且会发生嵌套抛出。
...
- 线程调度的三种方法  
  ...
  - 挂起方法join()  
    ...
    - 多线程同步问题Java的解决方案  
      ...
      > 同步方法：当线程进入同步方法的时候，会获得同步方法所属对象的锁，一旦获得对象锁，则其他线程不能再执行被锁对象的其他任何同步方法。
      >
      > ​		synchronized 方法申明{}
      >
      > 同步块：synchronized(资源对象){
      >
      > ​		//需要进行同步的方法
      >
      > ​		}
